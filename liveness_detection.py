"""
Real MiniFASNet V2 from Silent-Face-Anti-Spoofing
Official implementation from: https://github.com/minivision-ai/Silent-Face-Anti-Spoofing
"""

import torch
import torch.nn as nn
import torch.nn.functional as F
from torch.nn import Linear, Conv2d, BatchNorm1d, BatchNorm2d, PReLU, Sequential, Module, AdaptiveAvgPool2d
import cv2
import numpy as np
...
def MiniFASNetV2(embedding_size=128, conv6_kernel=(7, 7),
    ...
    """Real MiniFASNet V2 architecture"""
...
def preprocess_face_for_fas(face_img, target_size=(80, 80)):
    """
    Preprocess face image for anti-spoofing model
    
    Args:
        face_img: numpy array (RGB)
...
class LivenessDetector:
    """
    Face anti-spoofing (liveness detection) using real MiniFASNet V2
    """
    def __init__(self, model_path, device='cpu'):
        self.device = torch.device(device)
        # Use (5, 5) kernel to match pretrained weights (not default 7, 7)
        # num_classes=3 (real/fake/uncertain) to match pretrained model
        self.model = MiniFASNetV2(embedding_size=128, conv6_kernel=(5, 5), num_classes=3).to(self.device)
        
        if not model_path or not Path(model_path).exists():
            raise FileNotFoundError(f"MiniFASNet V2 model not found at: {model_path}")
        ...
        try:
            ...
            self.model.eval()
            print(f"✅ MiniFASNet V2 loaded from {model_path}")
        except Exception as e:
            raise RuntimeError(f"Failed to load MiniFASNet V2 model: {e}")
    
    def predict(self, face_img):
        """
        Predict liveness score using MiniFASNet V2
        ...
        """
        # Preprocess
        img_tensor = preprocess_face_for_fas(face_img).unsqueeze(0).to(self.device)
        ...
        with torch.no_grad():
            ...
            # Class 0: real, Class 1: fake (2 classes only)
            # Return probability of real
            liveness_score = probs[0, 0].item()
        
        return liveness_score


def create_liveness_detector(model_path, device='cpu'):
    """Factory function to create liveness detector - REQUIRES model_path"""
    if not model_path:
        raise ValueError("model_path is required for MiniFASNet V2 liveness detection")
